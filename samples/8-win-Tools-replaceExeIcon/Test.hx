...
class Test {
	...
	static function main() {		
		...
		var exefile = Dialogs.openFile("Select the executable file to update","",exefilter);			
		if (exefile==null) {
		...
		var icofile = Dialogs.openFile("Select a new icon file","",icofilter);
		if (icofile==null) {
		...
		// replace icon:
		if (win.Tools.replaceExeIcon(exefile[0],icofile[0]))
